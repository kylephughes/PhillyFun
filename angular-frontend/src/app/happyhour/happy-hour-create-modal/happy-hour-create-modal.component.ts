...
export class HappyHourCreateModalComponent implements OnInit {
  ...
  constructor(
    @Optional() @Inject(MAT_DIALOG_DATA) private editData: HappyHourModel,
  ...
  setEditOrNew = () => {
    this.editData.name == ""
      ...
      : ((this.title = "Edit -- " + this.editData.name),
  ...
  handleAddressChange(address) {
    this.form.controls["name"].setValue(address.name);
    this.form.controls["latitude"].setValue(address.geometry.location.lat());
    this.form.controls["longitude"].setValue(address.geometry.location.lng());
    this.form.controls["formattedAddress"].setValue(address.formatted_address);
  }
  ...
  submit() {
    ...
    if (this.editData.name == "") {
      this.happyhourServ
        .postNewHappyHour(this.form.value)
      ...
      this.happyhourServ
        .updateHappyHour(this.editData._id, this.form.value)
        .subscribe(apiResponse => {
          //  this.apiResponse = apiResponse;
          this.dataSent = false;
          //modify the config more
          this.snackbar.open(
            this.editData.name + " has been updated!",
            "Close",
